...
//         <button onClick={handleSetTasksPerPage}>Show 10 per page</button>
//       </div>
//     </div>
//   );
// };
...
const TaskList = ({ tasks, tasksPerPage, currentPage, setCurrentPage, onEdit, onDelete, onCreate, onRefresh, onSetTasksPerPage }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedTasks, setSelectedTasks] = useState([]);
  const [isTasksVisible, setTasksVisible] = useState(true);
  const [isDropdownOpen, setDropdownOpen] = useState(false);
  ...
  return (
    <div>
      ...
      <div className="tasks-per-page">
        <button onClick={() => onSetTasksPerPage(10)}>Show 10 Tasks</button>
      </div>
    </div>
  );
};
